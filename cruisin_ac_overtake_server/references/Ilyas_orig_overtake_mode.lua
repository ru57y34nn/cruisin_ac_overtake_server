...
function script.update(dt)
  ...
  local comboFadingRate = 0.05 * math.log(comboMeter) * math.lerp(1, 0.1, math.lerpInvSat(player.speedKmh, 80, 200)) + player.wheelsOutside/2
    ...
    if car.position:closerToThan(player.position, 10) then
      ...
      if not state.overtaken and not state.collided and state.drivingAlong then
        ...
        if posDot < -0.5 and state.maxPosDot > 0.5 then
          ...
          countDown = countDown + 10 * math.lerp(0.1, 1, math.lerpInvSat(player.speedKmh, 80, 200))
