...
-- local tokyoTurbo = 'http' .. 's://drive.google.com/drive/folders/1xvx032gfa6ccLHSsXRTGzO0LasNFq8tW/Wolf and Raven - Tokyo Turbo - 01 Tokyo Turbo.mp3'
...
-- local wanganLegends = 'http' .. 's://drive.google.com/drive/folders/1xvx032gfa6ccLHSsXRTGzO0LasNFq8tW/Wolf and Raven - Tokyo Turbo - 02 Wangan Legends.mp3'
...
local carNames = {}
carNames[1] = 'Toyota Sprinter Trueno GT-Apex Kouki Hatchback'
...
local gameStartPos = 0.0222
local checkpt1pos = 0.2324
local checkpt2pos = 0.4768
local checkpt3pos = 0.7108
local finishpos = 0.9630
...
if carName == carNames[1] then
    -- carNameIdx = 1
    storedScores.ae86Score = ac.storage('ae86Score', carPBs[1])
    carPBs[1] = storedScores.ae86Score:get()
    currentCarPB = carPBs[1]
    storedTimes.ae86Time = ac.storage('ae86Time', carTimePBs[1])
    carTimePBs[1] = storedTimes.ae86Time:get()

elseif carName == carNames[2] then
    storedScores.supraScore = ac.storage('supraScore', carPBs[2])
    carPBs[2] = storedScores.supraScore:get()
    currentCarPB = carPBs[2]
    storedTimes.supraTime = ac.storage('supraTime', carTimePBs[2])
    carTimePBs[2] = storedTimes.supraTime:get()

elseif carName == carNames[3] then
    storedScores.supra2Score = ac.storage('supra2Score', carPBs[3])
    carPBs[3] = storedScores.supra2Score:get()
    currentCarPB = carPBs[3]
    storedTimes.supra2Time = ac.storage('supra2Time', carTimePBs[3])
    carTimePBs[3] = storedTimes.supra2Time:get()

elseif carName == carNames[4] then
    storedScores.skylineScore = ac.storage('skylineScore', carPBs[4])
    carPBs[4] = storedScores.skylineScore:get()
    currentCarPB = carPBs[4]
    storedTimes.skylineTime = ac.storage('skylineTime', carTimePBs[4])
    carTimePBs[4] = storedTimes.skylineTime:get()

elseif carName == carNames[5] then
    storedScores.rx7Score = ac.storage('rx7Score', carPBs[5])
    carPBs[5] = storedScores.rx7Score:get()
    currentCarPB = carPBs[5]
    storedTimes.rx7Time = ac.storage('rx7Time', carTimePBs[5])
    carTimePBs[5] = storedTimes.rx7Time:get()

elseif carName == carNames[6] then
    storedScores.mx5Score = ac.storage('mx5Score', carPBs[6])
    carPBs[6] = storedScores.mx5Score:get()
    currentCarPB = carPBs[6]
    storedTimes.mx5Time = ac.storage('mx5Time', carTimePBs[6])
    carTimePBs[6] = storedTimes.mx5Time:get()


elseif carName == carNames[7] then
    storedScores.stiScore = ac.storage('stiScore', carPBs[7])
    carPBs[7] = storedScores.stiScore:get()
    currentCarPB = carPBs[7]
    storedTimes.stiTime = ac.storage('stiTime', carTimePBs[7])
    carTimePBs[7] = storedTimes.stiTime:get()


elseif carName == carNames[8] then
    storedScores.evoScore = ac.storage('evoScore', carPBs[8])
    carPBs[8] = storedScores.evoScore:get()
    currentCarPB = carPBs[8]
    storedTimes.evoTime = ac.storage('evoTime', carTimePBs[8])
    carTimePBs[8] = storedTimes.evoTime:get()

elseif carName == carNames[9] then
    storedScores.mr2Score = ac.storage('mr2Score', carPBs[9])
    carPBs[9] = storedScores.mr2Score:get()
    currentCarPB = carPBs[9]
    storedTimes.mr2Time = ac.storage('mr2Time', carTimePBs[9])
    carTimePBs[9] = storedTimes.mr2Time:get()

elseif carName == carNames[10] then
    storedScores.s2000Score = ac.storage('s2000Score', carPBs[10])
    carPBs[10] = storedScores.s2000Score:get()
    currentCarPB = carPBs[10]
    storedTimes.s2000Time = ac.storage('s2000Time', carTimePBs[10])
    carTimePBs[10] = storedTimes.s2000Time:get()
...
function script.update(dt)
    ...
    -- ac.debug("ae86 high score", carPBs[1])
    -- ac.debug("supra high score", carPBs[2])
    -- ac.debug("supra3.0 high score", carPBs[3])
    -- ac.debug("skyline high score", carPBs[4])
    -- ac.debug("rx7 high score", carPBs[5])
    -- ac.debug("m3 high score", carPBs[6])
    -- ac.debug("sti high score", carPBs[7])
    -- ac.debug("evo high score", carPBs[8])
    ...
    local sim = ac.getSim()
    if player.splinePosition > gameStartPos and not checkpoint10 then
        if not raceBegin and gameOverMessage == 0 then
            if muteToggle then
                mediaPlayer1:play()
            end
        ...
        if countDown > 0 then
            if player.splinePosition < finishpos then
                ...
                for i = 1, ac.getSim().carsCount do
                    ...
                    if car.position:closerToThan(player.position, 10) then
                        ...
                        if not state.overtaken and not state.collided and state.drivingAlong then
                            speedDiff = math.ceil(math.abs((player.speedKmh - car.speedKmh)*0.621))

                            ac.debug("speed difference is ", speedDiff)
                            ...
                            if posDot < -0.5 and state.maxPosDot > 0.5 then
                                ...
                                if collisionTimer > 0 then
                                    totalScore = totalScore + collisionOvertakePts * speedDiff
                                    addMessage('OVERTAKE 0.25x: + ' .. collisionOvertakePts * speedDiff .. ' pts', 2)
                                    -- collisionTimer = 0
                                -- end  
                                elseif car.position:closerToThan(player.position, 3) then
                                    totalScore = totalScore + closeOvertakePts * speedDiff
                                    -- addMessage(CloseMessages[math.random(#CloseMessages)], 1)
                                    addMessage('OVERTAKE 5X: + ' .. closeOvertakePts * speedDiff .. ' pts', 1)
                                else
                                    totalScore = totalScore + overtakePts * speedDiff
                                    addMessage('OVERTAKE 1x: + ' .. overtakePts * speedDiff .. ' pts', 0)
                                end
                ...
                if not checkpoint10 then
                    ...
                    -- testSecondsString = string.sub(string.format("%.2f", totalTimer/100), -2, -1)
                    -- if totalTimer/100 > 59 then
                    --     testSeconds = testSeconds - math.floor(testSeconds)
                    -- end
                    ...
                    if carName == carNames[1] then
                        if finalScore > carPBs[1] then
                            ...
                            storedScores.ae86Score:set(carPBs[1])
                    ...
                    elseif carName == carNames[2] then
                        if finalScore > carPBs[2] then
                            ...
                            storedScores.supraScore:set(carPBs[2])
                            currentCarPB = finalScore
                            if not highScoreChatMsgSent then
                                ac.sendChatMessage('Reached the Goal' .. ' in ' .. math.floor(minutesTime).. ":" .. string.format("%02d", math.floor(secondsTime)).. ":" .. string.sub(string.format("%.2f", subSeconds), -2, -1) .. ' and has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' ..carName)
                                highScoreChatMsgSent = true
                    ...
                    elseif carName == carNames[3] then
                        if finalScore > carPBs[3] then
                            ...
                            storedScores.supra2Score:set(carPBs[3])
                            currentCarPB = finalScore
                            if not highScoreChatMsgSent then
                                ac.sendChatMessage('Reached the Goal' .. ' in ' .. math.floor(minutesTime).. ":" .. string.format("%02d", math.floor(secondsTime)).. ":" .. string.sub(string.format("%.2f", subSeconds), -2, -1) .. ' and has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' ..carName)
                                highScoreChatMsgSent = true
                    ...
                    elseif carName == carNames[4] then
                        if finalScore > carPBs[4] then
                            ...
                            storedScores.skylineScore:set(carPBs[4])
                            currentCarPB = finalScore
                            if not highScoreChatMsgSent then
                                ac.sendChatMessage('Reached the Goal' .. ' in ' .. math.floor(minutesTime).. ":" .. string.format("%02d", math.floor(secondsTime)).. ":" .. string.sub(string.format("%.2f", subSeconds), -2, -1) .. ' and has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' ..carName)
                                highScoreChatMsgSent = true
                    ...
                    elseif carName == carNames[5] then
                        if finalScore > carPBs[5] then
                            ...
                            storedScores.rx7Score:set(carPBs[5])
                            currentCarPB = finalScore
                            if not highScoreChatMsgSent then
                                ac.sendChatMessage('Reached the Goal' .. ' in ' .. math.floor(minutesTime).. ":" .. string.format("%02d", math.floor(secondsTime)).. ":" .. string.sub(string.format("%.2f", subSeconds), -2, -1) ..' and has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' ..carName)
                                highScoreChatMsgSent = true
                    ...
                    elseif carName == carNames[6] then
                        if finalScore > carPBs[6] then
                            ...
                            storedScores.mx5Score:set(carPBs[6])
                            currentCarPB = finalScore
                            if not highScoreChatMsgSent then
                                ac.sendChatMessage('Reached the Goal' .. ' in ' .. math.floor(minutesTime).. ":" .. string.format("%02d", math.floor(secondsTime)).. ":" .. string.sub(string.format("%.2f", subSeconds), -2, -1) .. ' and has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' ..carName)
                                highScoreChatMsgSent = true
                    ...
                    elseif carName == carNames[7] then
                        if finalScore > carPBs[7] then
                            ...
                            storedScores.stiScore:set(carPBs[7])
                            currentCarPB = finalScore
                            if not highScoreChatMsgSent then
                                ac.sendChatMessage('Reached the Goal' .. ' in ' .. math.floor(minutesTime).. ":" .. string.format("%02d", math.floor(secondsTime)).. ":" .. string.sub(string.format("%.2f", subSeconds), -2, -1) .. ' and has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' ..carName)
                                highScoreChatMsgSent = true
                    ...
                    elseif carName == carNames[8] then
                        if finalScore > carPBs[8] then
                            ...
                            storedScores.evoScore:set(carPBs[8])
                            currentCarPB = finalScore
                            if not highScoreChatMsgSent then
                                ac.sendChatMessage('Reached the Goal' .. ' in ' .. math.floor(minutesTime).. ":" .. string.format("%02d", math.floor(secondsTime)).. ":" .. string.sub(string.format("%.2f", subSeconds), -2, -1) .. ' and has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' ..carName)
                                highScoreChatMsgSent = true
                    ...
                    elseif carName == carNames[9] then
                        if finalScore > carPBs[9] then
                            carPBs[9] = math.ceil(finalScore)
                            storedScores.mr2Score:set(carPBs[9])
                            currentCarPB = finalScore
                            if not highScoreChatMsgSent then
                                ac.sendChatMessage('Reached the Goal' .. ' in ' .. math.floor(minutesTime).. ":" .. string.format("%02d", math.floor(secondsTime)).. ":" .. string.sub(string.format("%.2f", subSeconds), -2, -1) .. ' and has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' ..carName)
                                highScoreChatMsgSent = true
                    ...
                    elseif carName == carNames[10] then
                        if finalScore > carPBs[10] then
                            carPBs[10] = math.ceil(finalScore)
                            storedScores.s2000Score:set(carPBs[10])
                            currentCarPB = finalScore
                            if not highScoreChatMsgSent then
                                ac.sendChatMessage('Reached the Goal' .. ' in ' .. math.floor(minutesTime).. ":" .. string.format("%02d", math.floor(secondsTime)).. ":" .. string.sub(string.format("%.2f", subSeconds), -2, -1) .. ' and has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' ..carName)
                                highScoreChatMsgSent = true
                    ...
                    --     storedScores.playerscore:set(personalBest)
                    --     storedScores.carName:set(carName)
                    if muteToggle then
                        mediaPlayer3:play()
                    end
                    checkpoint10 = true
            ...
            elseif carName == carNames[10] then
                if finalScore > carPBs[10] then
                    carPBs[10] = math.ceil(finalScore)
                    storedScores.s2000:set(carPBs[10])
                    currentCarPB = finalScore
                    if not highScoreChatMsgSent then
                        ac.sendChatMessage('Ran out of time, but has a new Highscore of ' .. math.ceil(finalScore) .. ' pts in the ' .. carName)
                        highScoreChatMsgSent = true
    ...
    ac.debug("subseconds", subSeconds)
    ac.debug("formatted subseconds", string.sub(string.format("%.2f", subSeconds), -2, -1))
    
    -- testSeconds = totalTimer/60
    -- if testSeconds > 59 then
    --     testSeconds = testSeconds - math.floor(testSeconds)
    -- end
    ...
    ac.debug("test seconds", string.format("%02d", math.floor(secondsTime)))
    ac.debug("seconds time", secondsTime)
    ...
    -- offRoadMessageTimer = offRoadMessageTimer + dt
...
local gobigfontscale = 0.08
local gobigfont_sizex = gobigfontscale*uiState.windowSize.x/screen_ratio
local gobigfont_sizey = gobigfontscale*uiState.windowSize.y
local gobig_fontsize = vec2(gobigfont_sizex, gobigfont_sizey)
...
function script.drawUI()
    ...
    if UIToggle then
        ...
        ui.drawImage('https://cdn.discordapp.com/attachments/1193340713123983500/1199189350429241497/cruisin-ac-logo.png?ex=673d531d&is=673c019d&hm=558d60175ab39d71d2d2406585e18a23feefd0a0900653f5b40dce7e04b91025&', vec2(-0.05, font_sizey*2), vec2(uiState.windowSize.x * 0.15, font_sizey*5), rgbm.colors.white, vec2(0,0), vec2(1,1), ui.ImageFit.Fit)
        ...
        checkpt1flag = math.applyLag(checkpt1flag, player.splinePosition > checkpt1pos and player.splinePosition < checkpt1pos + checkptflagdist and gameOverMessage == 0 and 1 or 0, 0.1, uiState.dt)
        checkpt2flag = math.applyLag(checkpt2flag, player.splinePosition > checkpt2pos and player.splinePosition < checkpt2pos + checkptflagdist and gameOverMessage == 0 and 1 or 0, 0.1, uiState.dt)
        checkpt3flag = math.applyLag(checkpt3flag, player.splinePosition > checkpt3pos and player.splinePosition < checkpt3pos + checkptflagdist and gameOverMessage == 0 and 1 or 0, 0.1, uiState.dt)
         finishflag = math.applyLag(finishflag, player.splinePosition >= finishpos and gameOverMessage == 0 and 1 or 0, 0.1, uiState.dt)
       ...
       local laptime_windowy_scale = 5*bigfont_sizey
        ...
        ui.beginTransparentWindow("gameOverWindow", vec2(uiState.windowSize.x * 0.5 - 4.5*gobigfont_sizex, uiState.windowSize.y * 0.5 - 2*gobigfont_sizey), vec2(2000, 2000), false)
        -- ui.beginOutline()
        ui.pushStyleVar(ui.StyleVar.Alpha, gameOverMessage)
        ui.pushACFont("LEDCalculator")
        ui.acText('GAME OVER!', gobig_fontsize, 0, rgbm(1.0, 0, 0, 1))
        ui.popACFont()
        ui.popStyleVar()
        ui.endTransparentWindow()
